...
(function () {
	const loadPackage = function (metadata) {
		...
		const PACKAGE_NAME = 'dist/subtitles-octopus-worker.data';
...
const out = Module.print || (typeof console !== 'undefined'
	? console.log.bind(console)
	: typeof print !== 'undefined'
		? print
		: null);

var err = Module.printErr || (typeof printErr !== 'undefined'
	? printErr
...
function setValue(ptr, value, type, noSafe) {
	...
	switch (type) {
		...
		case 'i64':
			(tempI64 = [
				value >>> 0, ((tempDouble = value), +Math_abs(tempDouble) >= 1
					? (tempDouble > 0
...
function allocate(slab, types, allocator, ptr) {
	...
	const singleType = typeof types === 'string'
		? types
		: null;
	let ret;
	if (allocator == ALLOC_NONE) {
		...
		ret = [_malloc, stackAlloc, dynamicAlloc][allocator](Math.max(size, singleType
			? 1
			: types.length));
	}
	if (zeroinit) {
...
const UTF16Decoder = typeof TextDecoder !== 'undefined'
...
const DYNAMIC_BASE = 5601424;
var DYNAMICTOP_PTR = 358512;

const TOTAL_STACK = 5242880;

const INITIAL_TOTAL_MEMORY = Module.TOTAL_MEMORY || 16777216;
	...
	function instantiateAsync() {
		if (!Module.wasmBinary && typeof WebAssembly.instantiateStreaming === 'function' && !isDataURI(wasmBinaryFile) && typeof fetch === 'function') {
			fetch(wasmBinaryFile, {
				...
				return WebAssembly.instantiateStreaming(response, info).then(receiveInstantiatedSource, (reason) => {
...
var Browser = {
	...
	calculateMouseEvent: function (event) {
		if (Browser.pointerLock) {
			...
			const scrollX = typeof window.scrollX !== 'undefined'
				? window.scrollX
				: window.pageXOffset;
...
var MEMFS = {
	...
	expandFileStorage: function (node, newCapacity) {
		const prevCapacity = node.contents
			? node.contents.length
			: 0;
		if (prevCapacity >= newCapacity) return;
		const CAPACITY_DOUBLING_MAX = 1024 * 1024;
		newCapacity = Math.max(newCapacity, (prevCapacity * (prevCapacity < CAPACITY_DOUBLING_MAX
			? 2
			: 1.125)) | 0);
	...
	node_ops: {
		getattr: function (node) {
			const attr = {};
			attr.dev = FS.isChrdev(node.mode)
				? node.id
				: 1;
			attr.ino = node.id;
			attr.mode = node.mode;
...
var FS = {
	...
	createFolder: function (parent, name, canRead, canWrite) {
		const path = PATH.join2(typeof parent === 'string'
			? parent
...
var SYSCALLS = {
	...
	doStat: function (func, path, buf) {
		...
		HEAP32[(buf + 32) >> 2] = 0;
		(tempI64 = [
			stat.size >>> 0, ((tempDouble = stat.size), +Math_abs(tempDouble) >= 1
				? (tempDouble > 0
					? (Math_min(+Math_floor(tempDouble / 4294967296), 4294967295) | 0) >>> 0
					: ~~+Math_ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0)
				: 0),
		...
		HEAP32[(buf + 76) >> 2] = 0;
		(tempI64 = [
			stat.ino >>> 0, ((tempDouble = stat.ino), +Math_abs(tempDouble) >= 1
				? (tempDouble > 0
					? (Math_min(+Math_floor(tempDouble / 4294967296), 4294967295) | 0) >>> 0
					: ~~+Math_ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0)
				: 0),
...
function ___syscall140(which, varargs) {
	...
	try {
		...
		(tempI64 = [
			stream.position >>> 0, ((tempDouble = stream.position), +Math_abs(tempDouble) >= 1
				? (tempDouble > 0
					? (Math_min(+Math_floor(tempDouble / 4294967296), 4294967295) | 0) >>> 0
					: ~~+Math_ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0)
				: 0),
...
function ___syscall220(which, varargs) {
	...
	try {
		...
		while (idx < stream.getdents.length && pos + struct_size <= count) {
			...
			(tempI64 = [
				id >>> 0, ((tempDouble = id), +Math_abs(tempDouble) >= 1
					? (tempDouble > 0
						? (Math_min(+Math_floor(tempDouble / 4294967296), 4294967295) | 0) >>> 0
						: ~~+Math_ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0)
					: 0),
			...
			(tempI64 = [
				((idx + 1) * struct_size) >>> 0, ((tempDouble = (idx + 1) * struct_size), +Math_abs(tempDouble) >= 1
					? (tempDouble > 0
						? (Math_min(+Math_floor(tempDouble / 4294967296), 4294967295) | 0) >>> 0
						: ~~+Math_ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0)
					: 0),
...
function ___syscall9(which, varargs) {
	SYSCALLS.varargs = varargs;
	try {
		const oldpath = SYSCALLS.get();
		const newpath = SYSCALLS.get();
		return -31;
	} catch (e) {
		if (typeof FS === 'undefined' || !(e instanceof FS.ErrnoError)) abort(e);
		return -e.errno;
...
function emscripten_realloc_buffer(size) {
	...
	try {
		const result = wasmMemory.grow((size - oldSize) / 65536);
		if (result !== (-1 | 0)) {
			buffer = wasmMemory.buffer;
			return true;
		}
...
const _libassjs_create_track = (Module._libassjs_create_track = function () {
	return Module.asm.$.apply(null, arguments);
});

const _libassjs_free_track = (Module._libassjs_free_track = function () {
	return Module.asm.aa.apply(null, arguments);
});

const _libassjs_init = (Module._libassjs_init = function () {
	return Module.asm.ba.apply(null, arguments);
});

const _libassjs_quit = (Module._libassjs_quit = function () {
	return Module.asm.ca.apply(null, arguments);
});

const _libassjs_render = (Module._libassjs_render = function () {
	return Module.asm.da.apply(null, arguments);
});

const _libassjs_resize = (Module._libassjs_resize = function () {
	return Module.asm.ea.apply(null, arguments);
});

const _main = (Module._main = function () {
...
self.writeFontToFS = function (font) {
	...
	const content = Module.readBinary(self.availableFonts[font]);
...
self.render = function (force) {
	...
	const renderResult = self._render(self.getCurrentTime() + self.delay, self.changed);
...
self.buildResultItem = function (ptr) {
	const bitmap = Module.getValue(ptr + 12, '*');
	const stride = Module.getValue(ptr + 8, 'i32');
	const w = Module.getValue(ptr + 0, 'i32');
	const h = Module.getValue(ptr + 4, 'i32');
	const color = Module.getValue(ptr + 16, 'i32');
	if (w == 0 || h == 0) {
		return null;
	}
...
function parseAss(content) {
	let m; let format; let lastPart; let parts; let key; let value; let tmp; let i; let j; let body;
	const sections = [];
