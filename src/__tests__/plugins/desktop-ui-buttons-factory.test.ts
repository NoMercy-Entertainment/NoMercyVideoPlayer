...
describe('subtitleSettingActions factory', () => {
	...
	it('covers every fontFamily entry with property=fontFamily', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const fontActions = actions.filter(subtitleSettingAction => subtitleSettingAction.property === 'fontFamily');
	...
	it('fontFamily action calls player.subtitleStyle with correct patch', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const fontAction = actions.find(subtitleSettingAction => subtitleSettingAction.property === 'fontFamily');
	...
	it('covers every textSize entry with property=fontSize', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const sizeActions = actions.filter(subtitleSettingAction => subtitleSettingAction.property === 'fontSize');
	...
	it('fontSize action calls player.subtitleStyle with correct patch', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const sizeAction = actions.find(subtitleSettingAction => subtitleSettingAction.property === 'fontSize' && subtitleSettingAction.value === 100);
	...
	it('covers every color entry for textColor with property=textColor', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const textColorActions = actions.filter(subtitleSettingAction => subtitleSettingAction.property === 'textColor');
	...
	it('textColor action calls player.subtitleStyle with correct patch', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const firstColor = actions.find(subtitleSettingAction => subtitleSettingAction.property === 'textColor');
	...
	it('covers every opacity entry for textOpacity with property=textOpacity', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const opacityActions = actions.filter(subtitleSettingAction => subtitleSettingAction.property === 'textOpacity');
	...
	it('textOpacity action calls player.subtitleStyle with correct patch', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const first = actions.find(subtitleSettingAction => subtitleSettingAction.property === 'textOpacity');
	...
	it('covers every edgeStyle entry with property=edgeStyle', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const edgeActions = actions.filter(subtitleSettingAction => subtitleSettingAction.property === 'edgeStyle');
	...
	it('edgeStyle action calls player.subtitleStyle with correct patch', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const first = actions.find(subtitleSettingAction => subtitleSettingAction.property === 'edgeStyle');
		expect(first).toBeDefined();
	...
	it('covers every color entry for backgroundColor with property=backgroundColor', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const bgActions = actions.filter(subtitleSettingAction => subtitleSettingAction.property === 'backgroundColor');
	...
	it('backgroundColor action calls player.subtitleStyle with correct patch', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const first = actions.find(subtitleSettingAction => subtitleSettingAction.property === 'backgroundColor');
		expect(first).toBeDefined();
	...
	it('covers every opacity entry for backgroundOpacity with property=backgroundOpacity', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const bgOpActions = actions.filter(subtitleSettingAction => subtitleSettingAction.property === 'backgroundOpacity');
	...
	it('backgroundOpacity action calls player.subtitleStyle with correct patch', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const first = actions.find(subtitleSettingAction => subtitleSettingAction.property === 'backgroundOpacity');
		expect(first).toBeDefined();
	...
	it('covers every color entry for areaColor with property=areaColor', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const areaActions = actions.filter(subtitleSettingAction => subtitleSettingAction.property === 'areaColor');
	...
	it('areaColor action calls player.subtitleStyle with correct patch', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const first = actions.find(subtitleSettingAction => subtitleSettingAction.property === 'areaColor');
		expect(first).toBeDefined();
	...
	it('covers every opacity entry for windowOpacity with property=windowOpacity', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const winActions = actions.filter(subtitleSettingAction => subtitleSettingAction.property === 'windowOpacity');
	...
	it('windowOpacity action calls player.subtitleStyle with correct patch', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const first = actions.find(subtitleSettingAction => subtitleSettingAction.property === 'windowOpacity');
		expect(first).toBeDefined();
	...
	it('includes a Reset entry with empty property', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const reset = actions.find(subtitleSettingAction => subtitleSettingAction.property === '');
	...
	it('Reset action calls player.subtitleStyle with defaultSubtitleStyles', () => {
		const player = makePlayer();
		const actions = subtitleSettingActions(player as never);
		const reset = actions.find(subtitleSettingAction => subtitleSettingAction.property === '' && subtitleSettingAction.label === 'Reset');
		expect(reset).toBeDefined();
